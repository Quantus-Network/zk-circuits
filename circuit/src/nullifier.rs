...
pub const SALT_NUM_TARGETS: usize = 1;
pub const SECRET_NUM_TARGETS: usize = 4;
pub const NONCE_NUM_TARGETS: usize = 1;
pub const FUNDING_ACCOUNT_NUM_TARGETS: usize = 4;
pub const PREIMAGE_NUM_TARGETS: usize = SALT_NUM_TARGETS + SECRET_NUM_TARGETS + NONCE_NUM_TARGETS + FUNDING_ACCOUNT_NUM_TARGETS;
...
impl Nullifier {
    pub fn new(secret: &[u8], funding_nonce: u32, funding_account: [u8; 32]) -> Self {
...
pub mod test_helpers {
    ...
    pub const FUNDING_ACCOUNT: &str = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    impl Default for Nullifier {
        fn default() -> Self {
            let secret = hex::decode(SECRET).unwrap();
            let account = hex::decode(FUNDING_ACCOUNT).unwrap();
            Self::new(secret.as_slice(), FUNDING_NONCE, <[u8; 32]>::try_from(account.as_slice()).unwrap())
        }
    }
...
pub mod tests {
    ...
    use crate::nullifier::test_helpers::SECRET;
