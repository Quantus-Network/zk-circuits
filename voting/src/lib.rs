...
use zk_circuits_common::{
	...
	utils::{felts_to_hashout, Digest, PrivateKey, DIGEST_NUM_FIELD_ELEMENTS, ZERO_DIGEST},
};
...
impl VoteTargets {
	pub fn new(builder: &mut CircuitBuilder<F, D>) -> Self {
		...
		let merkle_siblings: Vec<_> =
			(0..MAX_MERKLE_DEPTH).map(|_| builder.add_virtual_hash()).collect();
		let path_indices: Vec<_> =
			(0..MAX_MERKLE_DEPTH).map(|_| builder.add_virtual_bool_target_safe()).collect();
...
impl CircuitFragment for VoteCircuitData {
	...
	fn circuit(targets: &Self::Targets, builder: &mut CircuitBuilder<F, D>) {
		...
		for i in 0..MAX_MERKLE_DEPTH {
			...
			current_hash_targets =
				HashOutTarget { elements: next_hash_elements.try_into().unwrap() };
		}
	...
	fn fill_targets(
		...
		pw.set_hash_target(targets.proposal_id, felts_to_hashout(&self.public_inputs.proposal_id))?;
		pw.set_hash_target(
...
mod voting_tests {
	...
	fn create_test_inputs() -> VoteCircuitData {
		...
		let voter_private_key: PrivateKey =
			bytes_to_felts(&private_keys_for_tree[0]).try_into().unwrap();
		...
		let public_inputs = VotePublicInputs { proposal_id, merkle_root: root, vote, nullifier };
	...
	fn test_invalid_merkle_proof() -> anyhow::Result<()> {
		...
		assert!(proof_result.is_err(), "Proof generation should have failed but it succeeded");
		Ok(())
	}
	...
	fn test_completely_invalid_proof() -> anyhow::Result<()> {
		...
		inputs.private_inputs.merkle_siblings =
			vec![[F::from_canonical_u64(67890); 4], [F::from_canonical_u64(11111); 4]];
