...
/// The default depth of the tree of the aggregated proof, counted as the longest path of edges
/// between the leaf nodes and the root node.
...
impl TreeAggregationConfig {
	pub fn new(tree_branching_factor: usize, tree_depth: u32) -> Self {
		...
		Self { num_leaf_proofs, tree_branching_factor, tree_depth }
	}
}
...
fn aggregate_chunk(
	...
	let aggregated_proof = AggregatedProof { proof, circuit_data };
	Ok(aggregated_proof)
}
...
mod tests {
	...
	fn prove_square(value: F) -> AggregatedProof<F, C, D> {
		...
		AggregatedProof { proof, circuit_data }
	}
