...
//! 1. Initializing the verifier from pre-built circuit data via
//!    [`WormholeVerifier::new_from_files(...)`] or [`WormholeVerifier::new_from_bytes()`].
...
impl WormholeVerifier {
	...
	pub fn new_from_bytes(
		...
		let circuit_data = VerifierCircuitData { verifier_only, common };
	...
	pub fn new_from_files(
		...
		let verifier_only = VerifierOnlyCircuitData::from_bytes(verifier_bytes).map_err(|e| {
			anyhow!("Failed to deserialize verifier data from {:?}: {}", verifier_data_path, e)
		})?;
