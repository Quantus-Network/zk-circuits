...
impl TestInputs for CircuitInputs {
	fn test_inputs() -> Self {
		...
		let root_hash =
			hex::decode(DEFAULT_ROOT_HASH.trim()).unwrap().as_slice().try_into().unwrap();

		let funding_account = BytesDigest::from(DEFAULT_FUNDING_ACCOUNT);
		let nullifier = Nullifier::from_preimage(&secret, DEFAULT_TRANSFER_COUNT).hash.into();
...
pub mod storage_proof {
	...
	impl TestInputs for ProcessedStorageProof {
		fn test_inputs() -> Self {
			let proof = DEFAULT_STORAGE_PROOF.map(|node| hex::decode(node).unwrap()).to_vec();
