...
use zk_circuits_common::{
	...
	utils::{bytes_to_felts, felts_to_bytes, string_to_felt, Digest},
};
...
impl Nullifier {
	pub fn new(digest: BytesDigest, secret: &[u8], transfer_count: u64) -> Self {
		...
		Self { hash, secret, transfer_count }
	}
...
impl ByteCodec for Nullifier {
	...
	fn from_bytes(slice: &[u8]) -> anyhow::Result<Self> {
		...
		Ok(Self { hash, secret, transfer_count })
	}
}
...
impl From<&CircuitInputs> for Nullifier {
	fn from(inputs: &CircuitInputs) -> Self {
		Self::new(inputs.public.nullifier, &inputs.private.secret, inputs.private.transfer_count)
	}
}
...
impl CircuitFragment for Nullifier {
	...
	fn circuit(
		&Self::Targets { hash, ref secret, transfer_count }: &Self::Targets,
