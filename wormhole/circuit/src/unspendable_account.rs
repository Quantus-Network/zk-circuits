...
use zk_circuits_common::{
	...
	utils::{bytes_to_felts, felts_to_bytes, fixed_bytes_to_felts, string_to_felt, Digest},
};
...
impl UnspendableAccount {
	...
	pub fn from_secret(secret: &[u8]) -> Self {
		...
		Self { account_id, secret: secret_felts }
	}
}
...
impl ByteCodec for UnspendableAccount {
	...
	fn from_bytes(slice: &[u8]) -> anyhow::Result<Self> {
		...
		Ok(Self { account_id, secret: preimage })
	}
}
...
impl CircuitFragment for UnspendableAccount {
	...
	/// Builds a circuit that asserts that the `unspendable_account` was generated from
	/// `H(H(salt+secret))`.
	fn circuit(
		&Self::Targets { account_id, ref secret }: &Self::Targets,
