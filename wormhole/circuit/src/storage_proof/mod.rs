...
use zk_circuits_common::{
	...
	utils::{bytes_to_felts, BYTES_PER_ELEMENT},
};
...
impl StorageProofTargets {
	pub fn new(builder: &mut CircuitBuilder<F, D>) -> Self {
		...
		let indices: Vec<_> = (0..MAX_PROOF_LEN).map(|_| builder.add_virtual_target()).collect();
...
impl StorageProof {
	pub fn new(
		...
		let proof: Vec<Vec<F>> =
			processed_proof.proof.iter().map(|node| bytes_to_felts(node)).collect();
		...
		StorageProof { proof, indices, root_hash, leaf_inputs }
	}
}
...
impl CircuitFragment for StorageProof {
	...
	fn circuit(
		...
		// The first node should be the root node so we initialize `prev_hash` to the provided
		// `root_hash`.
		...
		for i in 0..MAX_PROOF_LEN {
			...
			let mut found_hash =
				vec![builder.zero(), builder.zero(), builder.zero(), builder.zero()];
	...
	fn fill_targets(
		...
		pw.set_target(targets.leaf_inputs.transfer_count, self.leaf_inputs.transfer_count)?;
		...
		pw.set_target_arr(&targets.leaf_inputs.funding_amount, &self.leaf_inputs.funding_amount)?;
...
fn slice_to_hashout(slice: &[u8]) -> HashOut<F> {
	...
	HashOut { elements: elements.try_into().unwrap() }
}
